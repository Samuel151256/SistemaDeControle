...
def consultar_produto_id(produtos, id):
  for produto in produtos:
    if produto.id == id:
      print(f'ID: {produto,id}, Nome: {produto.nome}, Categoria: {produto.categoria_id}, Quantidade: {produto.quantidade}, Preco: {produto.preco}')
      return
...
def registrar_movimentacao(movimentos, contador_movimentos, produtos):
  ...
  tipo_movimentacao = input('Tipo de movimentacao: \nE - entrada\nS - Saída: ').strip().upper()
...
def consultar_movimentacoes(movimentacoes, produto_id):
  print("Movimentações do produto ID {produto_id}: ")
